...
Вторая часть датасета состоит из известного набора данных IAM. Это набор рукописных слов на английском языке.

**Public.** Публичный лидерборд так же рассчитывается на части датасетов тетрадей и IAM.

**Private.** Приватный тестовый датасет скрыт от участников. Это тоже набор данных для распознавания текста, в похожем на обучающий датасет формате. Мы, однако, не предоставляем информации о деталях датасета.
