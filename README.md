...
1) общие параметры (shared weights) должны составлять **не менее 30%** всех параметров модели; *пояснение: если ![image](https://latex.codecogs.com/svg.image?N) - суммарное число параметров моделей, решающих **4** задачи, а ![image](https://latex.codecogs.com/svg.image?M) - число общих параметров этих моделей (то есть одинаковых и по значению, и архитектурно), тогда необходимо, чтобы ![image](https://latex.codecogs.com/svg.image?\frac{M}{N}&space;\geq&space;0.3))*; 
...
При невыполнении хотя бы одного требования выше модель считается решающей задачу (или задачи) по отдельности.
