...
Для того чтобы представленная командой/участником архитектура считалась единой (multitask-модель), необходимо, чтобы общие для всех модальностей параметры (shared weights) составляли **не менее 30%** всех параметров модели (если ![image](https://latex.codecogs.com/svg.image?N) - суммарное число параметров моделей, решающих **4** задачи, а ![image](https://latex.codecogs.com/svg.image?M) - число общих параметров этих моделей, то есть одинаковых и по значению, и архитектурно, тогда необходимо выполнение требования ![image](https://latex.codecogs.com/svg.image?\frac{M}{N} \geq 0.3)). Эти параметры не должны иметь исключительно номинальный характер — напротив, должны значимым образом использоваться во время совершения предсказания моделью и положительным образом влиять на качество её работы. В противном случае модель считается решающей задачу (или задачи) по отдельности.
