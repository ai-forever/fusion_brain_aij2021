...
По данной [ссылке](https://drive.google.com/uc?export=download&id=1nxDL9NvgSyC_-LAIVgqnKMJzIOQ-UH5m) находится архив `baseline_submission.zip`, в котором содержатся примеры загружаемого решения, входных данных для модели и правильных ответов. Архив состоит из следующих файлов:

* `input` - примеры входных данных для каждого из заданий. Этой папки в загружаемом на платформу архиве с решением быть не должно, она подкладывается системой в контейнер с загруженной моделью участника;
* `output` - примеры предсказаний модели для файлов из папки `input`. Это случайные предсказания, и они указывают только на верный формат, который ожидается от модели участников. Этой папки в загружаемом архиве с решением быть не должно: она формируется во время запуска модели в контейнере;
* `true` - примеры файлов с верными ответами для каждой задачи, с ними сравниваются предсказания из `output` во время подсчета метрики. Этой папки в загружаемом архиве с решением быть не должно.

Следующие файлы в загружаемом архиве необходимы для формирования предсказаний модели:
...
В папке `fb_baseline` находится наше базовое решение по всем четырем трекам. Это решение основано на концепте мультимодальной модели из `fb_concept`. В ноутбуке `fb_baseline/FBC_baseline.ipynb` находится полный код с формированием датасета, архитектурой модели и логикой обучения. В папке `fb_baseline/fb_utils` - вспомогательный набор скриптов.

* `BLEU.py` и `c2c_eval.py` - вспомогательные функции для инференса и рассчета метрики в задаче C2C
* `detection_vqa.py` - вынесена функция потерь, слои и вспомогательные функции для задач zsOD и VQA
* `download.py` - скрипт загрузки файлов для обучения
* `handwritten.py` - вспомогательные функции для задачи HTR
* `metrics.py` - подсчет метрик всех 4 задач
