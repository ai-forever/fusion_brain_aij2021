...
По данной [ссылке]() находится архив `baseline_submission.zip`, в котором содержатся примеры загружаемого решения, входных данных для модели и правильных ответов. Архив состоит из следующих файлов:

* `input` - в этой папке содержится пример входных данных для каждого из заданий. Этой папки в загружаемом на платформу архиве с решением быть не должно, она подкладывается системой в контейнер с загруженной моделью участника;
* `output` - в этой папке содержится пример предсказаний модели для файлов из папки `input`. Это случайные предсказания, и они указывают только на верный формат, который ожидается от модели участников. Этой папки в загружаемом архиве с решением быть не должно: она формируется во время запуска модели в контейнере;
* `true` - в этой папке содержится пример файлов с верными ответами для каждой задачи, с ними сравниваются предсказания из `output` во время подсчета метрики. Этой папки в загружаемом архиве с решением быть не должно.
