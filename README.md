...
По данной [ссылке](https://disk.yandex.ru/d/s-Hhy1x0Zy83dA) находится архив ```baseline_submission.zip``` - в нем содержатся пример загружаемого решения, пример входных данных для модели и пример правильных ответов. Если открыть этот архив, то его контент состоит из следующих файлов:

 * ```input``` - в этой папке содержится пример public test файлов. Этой папки в загружаемом на платформу архиве с решением быть не должно, она подкладывается системой в контейнер с загруженной моделью участника.
 ...
 * ```true``` -  в этой папке содержится пример public true файлов, с ними сравниваются предсказания из output во время подсчета метрики. Этой папки в загружаемом архиве с решением быть не должно.

Следующие файлы уже должны присутствовать в загружаемом архиве с решением.

 * ```metadata.json``` - необходимый файл для каждого решения, в котором должны быть пути к образу и скрипту выполнения модели.
 * ```run.py``` - основной скрипт для эвалуации модели.
 * ```last.pt``` - веса модели, которые подгружаются во время исполнения ```run.py```.
 * ```utils``` - вспомогательные скрипты для ```run.py```. Содержит два файла:
   *  ```dataset.py``` - вынесен код для формирования класса DatasetRetriever и функция fb_collate_fn.
   *  ```fb_model.py``` - вынесен весь код для класса модели.
 * ```gpt_weights``` - папка с необходимыми файлами для инициализации ```GPT2Tokenizer``` и ```GPT2Model```.
 * ```fb_utils``` - вспомогательный набор скриптов, повторяет аналогичную папку из ```fb_baseline``` в данном репозитории.
