...
* Как мы писали ранее - в паблик тесте есть изображения, повернутые на 90 градусов против часовой стрелки. Мы решили не давать повернутые изображения в прайват тесте. Поэтому мы выкладываем [ссылку](https://dsworks.s3pd01.sbercloud.ru/aij2021/sample_data/rotate.zip) на архив, который содержит файл `rotate.json`. В нем содержится список изображений из паблик теста, которые стоит повернуть на 90 градусов против часовой стрелки. Пример использования этого файла приведен в скрипте `rotate.py` внутри архива. Вы можете вставить подгрузку этого json-а в свой скрипт с решением, чтобы перевернуть нужные изображения во время подгрузки. Итак, резюмируя - в прайват тесте не будет повернутых изображений. Вы можете использовать `rotate.json` - это повлияет только на скор паблик текста. Для финальной метрики, рассчитанной по прайват тесту, он бесполезен. В представленном для обучения датасете русских тетрадей и предлагаемом IAM так же нет повернутых изображений, поэтому, как нам кажется, специально учить модель переворачивать изображения смысла нет.
