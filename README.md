...
В данном разделе будет добавляться важная информация по соревнованию.

* В раздел **Пример решения** добавлена ссылка на актульный пример сабмита, а так же ссылки на примеры входных/выходных данных.
* В тестовом датасете по HTR содержатся собранный и размеченный нами датасет школьных тетрадей. Он состоит из тетрадей по русскому и английскому языку. По ссылке из раздела  **Пример решения** можно скачать и посмотреть, как примерно выглядят данные. Подчеркнем, что в паблик тесте содержатся примеры с вертикальной ориентацией (повернутные на 90 градусов). Распределение данных в рукописных тетрадях на английском языке сильно отличается от предлагаемого нами для обучения датасета **IAM**. Поэтому советуем применять различные нормализации, аугментации и учиться не только на **IAM**.
...
По данной [ссылке](https://dsworks.s3pd01.sbercloud.ru/aij2021/baseline/sberai_baseline_ooc.zip) находится архив `sberai_baseline_ooc.zip`. В нем содержится пример загружаемого решения. Получаемую с помощью данного решения метрику можно найти в лидерборде соревнования по названию команды `sberaiooc`. Следующие файлы в загружаемом архиве необходимы для формирования предсказаний модели:
...
Так же мы выложили примеры данных, которые можно скачать по ссылкам. Это не настоящие данные, они нужны для того, чтобы примерно понять что может содержаться в нашем датасете. Советуем обратить внимание на `input/HTR/images`. Дополнительно к русскому, там содержатся примеры на английском.
...
* [true](https://dsworks.s3pd01.sbercloud.ru/aij2021/sample_data/true.zip) - примеры файлов с верными ответами для каждой задачи, с ними сравниваются предсказания из `output` во время подсчета метрики. 
