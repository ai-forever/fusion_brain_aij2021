...
Модель участника должна сделать предсказания на всех изображениях из папки images, и сгенерировать файл ```prediction_zsOD.json```. Это словарь формата ```{"0.jpg": [["красное яблоко, висящее на ветке", 0.5, 473.07, 395.93, 38.65, 28.67], ["девочка, кормящая слона", 0.6, 0.0, 101.15, 452.3, 319.43]], ...}```. Ключами являются названия файлов из папки images, значениями – предсказания модели для соответствующих изображений. Эти предсказания должны содержать обнаруженные классы вместе с координатами ограничивающих рамок для данного изображения: по ключу "название файла с изображением" – список предсказанных на нём объектов. Формат одного элемента в списке: ```["красное яблоко, висящее на ветке", 0.5, 473.07, 395.93, 38.65, 28.67]``` (шесть элементов, разделенных запятыми). Первый элемент – название класса, второй – score модели, затем идут 4 координаты bbox в формате ```xywh```. Во вложенном списке может содержаться неограниченное количество элементов – это те объекты, которые предсказала модель для данного изображения.
