...
Для того, чтобы представленная командой/участником архитектура считалась единой (multitask-модель), необходимо, чтобы общие для всех модальностей параметры (shared weights) составляли **не менее 30%** всех параметров модели. В противном случае модель считается решающей задач(у/и) по отдельности (unitask-модель). 
